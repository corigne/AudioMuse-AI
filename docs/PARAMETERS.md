...
| **Essentia MTG-Jamendo Mood Classifiers (opt-in)** |                                                                                      |                                        |
| `USE_ESSENTIA_MOOD_MODELS`                  | When `1`, per-axis mood scores (`aggressive`, `happy`, `party`, `relaxed`, `sad`, `danceable`) are derived from dedicated Essentia msd-musicnn classifiers instead of CLAP text-similarity. Produces better-calibrated, wider-spread mood scores across diverse genres. Requires `WITH_ESSENTIA_MOOD_MODELS=1` at Docker build time. | `0` |
